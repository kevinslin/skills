Shortcut: Run Pre-Commit Rules, Review, and Commit

Instructions:

Create a to-do list with the following items then perform all of them:

1. Follow the trigger:precommit-process steps to review and run pre-commit checks.

2. If there are issues, ask for guidance. Only do this is you really need to.
   If all looks correct commit, with a concise and thorough commit message.

3. If commit is successful and if there is a PR already filed, update the PR
   description.
