...
- The user explicitly asks for `flow-doc-v2`.

## Purpose

Flow docs v2 explain how given logic works without becoming a line-by-line code
...
3. Choose `{flow-name}` and copy `./references/flow-doc-v2/template.md` to `$DOCS_ROOT/flows/{flow-name}.md`.
4. Fill frontmatter:
...
16. Run validator from this skill root:
    - `python3 ./scripts/validate_flow_doc.py --kind v2 --doc "$DOCS_ROOT/flows/{flow-name}.md"`
...
6. Re-run the v2 validator before handoff.
...
- [ ] `validate_flow_doc.py --kind v2` passes.
