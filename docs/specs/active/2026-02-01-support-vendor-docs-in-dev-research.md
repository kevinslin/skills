# Execution Plan: Support Vendor Docs in specy
...
Extend the `specy` skill to support creating local vendor documentation (stored under `docs/vendor/{library}`) with required sections, input workflow, and file placement rules from issue #4.
...
### Architecture/Design
Add a new “Vendor Docs” document type to `active/specy/SKILL.md`, including input workflow (docs URL + library name), DOC_ROOT/LIB_ROOT definitions, required sections, and file placement rules. Introduce a new template under `active/specy/references/` to standardize vendor docs. Update directory structure and path convention sections accordingly.
...
### Phase 2: Update specy Skill Docs
...
- [x] Create `active/specy/references/vendor-doc.md` with README/reference/topics skeletons and required ending sections.
...
- [ ] `specy` skill includes vendor docs guidance and output structure.
