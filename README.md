...
- [fin](active/fin/SKILL.md): Finalize a completed task by landing or verifying the selected `gh`/`local` context, optionally targeting an explicit PR with `fin gh <number-or-url>`, archiving any active `specy` spec, refreshing and proving the local base, deterministically cleaning eligible local worktrees and branches, completing the active Codex thread's exactly linked pending Linear issue, and running `ag-learn` to propose retrospective improvements. Its bundled cleanup script is dry-run-first, full-commit-identity-locked, local-base-protected, journaled with filesystem identity, resumable across partial filesystem/registry removal, and forbidden from global worktree pruning or remote-branch mutation.
