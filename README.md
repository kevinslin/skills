...
- [mem](active/mem/SKILL.md): Resolve `.mem.yaml` knowledge bases for durable reads and writes, including per-base descriptions for routing, schemas, optional routing skills, and `path_style` conventions for dotted or directory-backed notes.
...
- [claw-integ](active/claw-integ/SKILL.md): Run live OpenClaw integration proof against a named claw gateway profile with showboat-v2 evidence and ffmpeg video proof.
...
- [specy](active/specy/SKILL.md): Create structured research documentation for codebase exploration and feature investigation, with lightweight contract checks for specs that change data/API/CLI/config or migration output. Covers architecture docs, research briefs, flow docs, state docs, service design docs, feature design docs, feature specs (execution plans), investigation specs, validation specs, recipes, vendor docs, and frequently asked questions (FAQ).
