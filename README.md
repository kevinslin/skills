...
- [ag-learn](active/ag-learn/SKILL.md): Improve skills from observed agent friction in sessions, PRs, or audits.
...
- [mem](active/mem/SKILL.md): Resolve `.mem.yaml` knowledge bases for durable reads and writes, including per-base schemas, optional routing skills, and `path_style` conventions for dotted or directory-backed notes.
...
- [sw-loop](active/sw-loop/SKILL.md): Swarm-based feature delivery workflow that coordinates a manager agent, spec work, implementation, review, cleanup, and verification across multiple subagents. Use when the user explicitly wants a swarm, parallelized feature work, or a managed loop that should use `$sw-ctrl`, `$specy`, `$dev.review`, and `$dev.loop` in that order.
...
- [dev.review](active/dev.review/SKILL.md): Multi-type review skill for code, developer docs, design docs/specs, architecture, UX, and other reviews. Use when the user asks for a review or critique (code review, docs review, design doc review, spec review, PR review).
...
- [fin](active/fin/SKILL.md): Finalize a completed task by landing or verifying the selected `gh`/`local` context, archiving any active `specy` spec, cleaning up linked worktrees and branches, refreshing `main`, and running `ag-learn` to propose retrospective improvements. Use at the end of implementation work when the requested scope is done and the checkout should be brought to a clean finished state; linked worktrees that are about to be deleted are reset and cleaned after final hooks run.
