...
- [fin](active/fin/SKILL.md): Finalize a completed task by landing or verifying the selected `gh`/`local` context, optionally targeting an explicit PR with `fin gh <number-or-url>`, archiving any active `specy` spec, cleaning up eligible linked worktrees and branches, refreshing `main`, and running `ag-learn` to propose retrospective improvements. Use at the end of implementation work when the requested scope is done and the checkout should be brought to a clean finished state; PR status reports must identify the target PR/branch/source, transient mergeability `UNKNOWN` states are cross-checked before conflict repair, and linked worktrees that are about to be deleted are reset and cleaned after final hooks run.
